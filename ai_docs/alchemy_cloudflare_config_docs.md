...
export const worker = await Worker("api", {
  ...
  compatibility: "node",                  // Compatibility preset (optional) â€” expands flags for common use-cases like Node.js compatibility
  ...
  sourceMap: true,
  // Optional runtime limits â€” increase only when necessary after monitoring
  limits: {
    cpu_ms: 50_000                        // Max CPU time in ms (default: 30_000)
  },
  // Other Advanced ðŸ§ª & commonly used options
...
**Benefits:**
- âœ… Automatic network optimization based on performance metrics
- âœ… Reduced latency for global users
- âœ… No infrastructure management required
- âœ… Seamless scaling across Cloudflare's network
...
- âœ… Service isolation and independent scaling
...
await Worker("api", {
  entrypoint: "./src/api.ts",
  bindings: { 
+    // âœ… Access the STORAGE DO hosted in the `data` worker
...
```ts
// alchemy.run.ts
// âœ… created a KV namespace
const cache = await KVNamespace("cache");

// âŒ but, forget to bind it to the worker
export const worker = await Worker("api");
```

```ts
// src/worker.ts
// âŒ Missing binding configuration
