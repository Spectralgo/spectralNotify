...
export const Route = createFileRoute("/_app/workflows/live")({
  component: LiveWorkflowsPage,
});

function LiveWorkflowsPage() {
  const { selectedWorkflowId, setSelectedWorkflowId } = useWorkflowsContext();

  const [searchQuery, setSearchQuery] = useState("");
  const [isLiveEnabled, setIsLiveEnabled] = useState(true);

  // Filter for active workflows (pending and in-progress)
  const { workflows: allWorkflows, isLoading } = useWorkflows({
    search: searchQuery,
  });
            ...
            <EmptyState
              description={
                searchQuery
                  ...
                  : "Active workflows will appear here"
              }
              icon={RadioTower}
              title={searchQuery ? "No matches found" : "No active workflows"}
            />
          ) : (
