...
function WorkflowListItem({
  ...
  return (
    <motion.button
      ...
      className={cn(
        "w-full rounded-lg border border-white/10 bg-gray-800/50 p-4 text-left transition-all hover:bg-gray-800/70",
    ...
    >
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center gap-2">
          <Progress className="h-2 flex-1" value={overallProgress} />
          <span className="whitespace-nowrap font-medium text-white text-xs">
            {Math.round(overallProgress)}%
          </span>
        </div>

        {/* Last Event */}
        <p className="line-clamp-1 text-gray-400 text-xs">{lastEvent}</p>
      </div>
    </motion.button>
  );
}
